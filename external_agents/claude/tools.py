...
SENDER_NAME = "Claude"
SENDER_CARD_URL = "http://localhost:8003/.well-known/agent-card.json"


# ─── Contact management ────────────────────────────────────────────

def get_my_contacts() -> str:
    """List all contacts in Claude's contact book with their type, description, and tags."""
...
def search_contacts_by_tag(tag: str) -> str:
    """Find contacts matching a specific tag (e.g. 'audio', 'foodie', 'friend')."""
...
async def add_contact(
    ...
    """Add a new contact to Claude's contact book."""
...
def remove_contact(name: str) -> str:
    """Remove a contact from Claude's contact book by name."""
...
async def send_message_to_contact(contact_name: str, message: str) -> str:
    ...
    if not contact:
        return (
            f"Contact '{contact_name}' not found in your contacts. "
            "Use get_my_contacts to see available contacts."
        )
...
def get_my_history(query: str) -> str:
    """Search Claude's interaction history for relevant past experiences."""
