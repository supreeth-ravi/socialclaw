...
SENDER_NAME = "Alice"
SENDER_CARD_URL = "http://localhost:8001/.well-known/agent-card.json"


# ─── Contact management ────────────────────────────────────────────

def get_my_contacts() -> str:
    """List all contacts in Alice's contact book with their type, description, and tags."""
...
async def add_contact(
    ...
    """Add a new contact to Alice's contact book.
...
def remove_contact(name: str) -> str:
    """Remove a contact from Alice's contact book by name."""
...
async def send_message_to_contact(contact_name: str, message: str) -> str:
    ...
    Looks up the contact in Alice's contact book, resolves their agent card,
...
def get_my_history(query: str) -> str:
    """Search Alice's interaction history for relevant past experiences.
